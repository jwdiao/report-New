  ...
  <div class="index">
    ...
    <div class="index_top">
      <div class="index_imgText">
        <span class="title" style="cursor:pointer;" @click="switchData('/CarsHLGV31')">南口•南大门车辆统计报表</span><!--南口•南大门-->
      </div>
              ...
              <div>
                <div class="index_center_top_main_Cars">
                  <div class="index_center_top_main_Cars_module carsN">
                    <p>进入数量&nbsp;&nbsp; <span class="index_left_top_main_Cars_module_num">{{carTimeNumObj.yet6EnterTruckNum||0}}</span></p>
                  </div>
                </div>
                <div class="index_center_top_main_Cars ">
                  <div class="index_center_top_main_Cars_module carsH">
                    <p class="leaveNumP">离开数量&nbsp;&nbsp; <span class="index_center_top_main_Cars_module_num">{{carTimeNumObj.yet6LeaveTruckNum||0}}</span></p><!--{{carAllData.strandedTruckNum}}-->
                  </div>
                </div>
              </div>
            </div>
          ...
          <div style=" border-radius:5px;background-color: rgba(42, 75, 133, 0.3);margin-right: 10px">
            ...
            <div class="index_center_top_main">
              <div>
                <div class="index_center_top_main_Cars">
                  <div class="index_center_top_main_Cars_module carsN">
                    <p>进入数量&nbsp;&nbsp; <span class="index_left_top_main_Cars_module_num">{{carTimeNumObj.yet9EnterCarNum||0}}</span></p>
                  </div>
                </div>
                <div class="index_center_top_main_Cars ">
                  <div class="index_center_top_main_Cars_module carsH">
                    <p>离开数量&nbsp;&nbsp; <span class="index_center_top_main_Cars_module_num">{{carTimeNumObj.yet9LeaveCarNum||0}}</span></p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div style=" border-radius:5px;background-color: rgba(42, 75, 133, 0.3);">
            ...
            <div class="index_center_top_main">
              <div>
                ...
                <div class="index_center_top_main_Cars ">
                  <div class="index_center_top_main_Cars_module carsH">
                    <p>离开数量&nbsp;&nbsp; <span class="index_center_top_main_Cars_module_num">{{carTimeNumObj.todayUpLeaveNum||0}}</span></p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div class="index_center_bottom_searchContent_mainAllsearch">
                <el-scrollbar class="message" style="overflow-x: hidden;">
                  <div class="title_message" v-for="(item,index) in allSearchCarListArr" :key="index">
                   ...
                   <span>{{item.plateTypeName}}</span>
                   <span>{{item.inDT}}</span>
                   <span>{{item.outDT == null ? '未离开' : item.outDT}}</span>
                    <span>{{item.memo === '内部车辆'? '自动放行' : '抬杆放行'}}</span>
              ...
              <div class="index_center_bottom_searchContent_main">
                <el-scrollbar class="message" style="overflow-x: hidden;">
                  <div class="title_message" v-for="(item,index) in noLeaveCarsList" :key="index">
                    ...
                    <span>{{item.memo === '内部车辆'?'自动放行':'手动抬杆'}}</span>
                     <span>{{item.customerName}}</span>
                  </div>
                </el-scrollbar>
              </div>
...
<script>
  import CarsManage from '../../components/CarsManage/CarsManage.vue'
  ...
  import {
    ...
    reqCarAllInfo,
    reqCenterTopSixData,
    reqLetBottomTenDayEchartsData,
    reqAllSearchData,
    reqCenterBottomAllSearchData
  }from'../../api'
  ...
  export default {
    ...
    methods: {
      ...
      async getThreeTimeCarsData(){
        const res = await reqCenterTopSixData()
        // const res = await http.get('http://10.88.195.89:8083/carPlater/getExceptionCarData')
       if(res.data.ret === '200'){
          this.carTimeNumObj = res.data
       }
      },
      //接口--中间--综合查询列表
      async getAllSearchListData(date,carNumber,carBelong,carType,isLeave,searchType,page,pageSize){
         const res = await reqCenterBottomAllSearchData(date,carNumber,carBelong,carType,isLeave,searchType,page,pageSize)
         // const res = await http.post('http://10.88.195.89:8083/carPlater/getSanyCarDataListOfNew',{'date':date,'ccode':carNumber,'carbelong':carBelong,'cartype':carType,'isoutstatus':isLeave,'searchType':searchType,'page':page,'pagesize':pageSize})
       if(res.data.ret === '200'){
        this.allSearchCarListArr = res.data.list
        this.allSearchTotals = res.data.total
       }
      },
      //接口---左侧
      async getCarAllData(carbelong,cartype,page,pagesize){
        const res = await reqCarAllInfo(carbelong,cartype,page.toString(),pagesize.toString())
       ...
       if(res.data.ret === '200'){
         ...
         this.noLeaveCarsList = res.data.strandedList
      ...
      async carRightForm (DateType) {
        // const res = await http.get('http://10.88.195.89:8083/carPlater/getExceptionCarOfmonth');
        const res = await reqLetBottomTenDayEchartsData()
        if (res && res.data.ret === '200') {
          switch (DateType){
            case 'SIXTIMETRUNK':  //晚6点
              const mySixTimeTrunk = echarts.init(document.getElementById('sixTimeTrunk'));
              this.chartsCData.xData = res.data.xAxis; // X轴
              this.chartsCData.insideCars = res.data.every6EnterTruckArr; // 进入
              this.chartsCData.externalCars = res.data.every6LeaveTruckArr; // 离开
            ...
            case 'NINETIMECARS'://晚9点后
              ...
              this.chartsBData.xData = res.data.xAxis; // X轴
              this.chartsBData.insideCars = res.data.every9EnterCarArr; // 进入
              this.chartsBData.externalCars = res.data.every9LeaveCarArr; // 离开
            ...
            case 'DAYTIMEAUTO'://自动放行
              ...
              this.chartsBData.xData = res.data.xAxis; // X轴
              this.chartsBData.insideCars = res.data.everyUpEnterArr; // 进入
              this.chartsBData.externalCars = res.data.everyUpLeaveArr; // 离开
    ...
    * */
      isLeaveSearchFn(val){
        ...
        if(obj.label === '已离开'){
          this.isLeaveStatus = '1'
        }else if(obj.label === '未离开'){
          this.isLeaveStatus = '0'
        }
      },
