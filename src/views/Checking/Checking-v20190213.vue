...
import AbnormalStatistics from '@/components/checking-v20190213/AbnormalStatistics'
import Attendance from '@/components/checking-v20190213/Attendance'
import EnergyStatistics from '@/components/checking-v20190213/EnergyStatistics'
import Checking from '@/components/checking-v20190213/Checking'
import MachingCenter from '@/components/checking-v20190213/MachingCenter'
...
export default {
  ...
  methods: {
    ...
    async getDayRightData (centername) {
      const res = await getWorkEchartsOfDay(centername)
    ...
    async getYearRightData (centername) {
      const res = await queryYear(centername)
      if (res.data && res.data.ret === '200') {
    ...
    selectedCenterName (centername) {
      ...
      if (this.$store.state.centername!=='') { // 子加工中心
        // 获取(旷工/迟到/离岗/未派工)数据
