<template>
	<div class="device" v-if="showFlag">
		<div class="device_head">
			<div class="leftInfo" @click="handleCloseAllScreen" style="cursor:pointer"><img src="./images/common_back.png"></div>
		...
		<div class="device_mainWrap">
			<div class="device_titleBox">
				...
				<img class="device_img" src="./images/DeviceConnect_title.png" />
	...
	export default {
		...
		methods: {
			...
			async getDeviceListFun() {
				...
				if(res && res.data.code === 200 && res.data.data && res.data.data.list.length){
						const deviceList = res.data.data.list
...
.deviceCondition{
	...
	.device {
		font-size: 12px;
		height: 100%;
		background-image: url(./images/common_bj.png);
		background-size: 100% 100%;
	}

	.device_head {
		background-image: url(./images/common_nav.png);
