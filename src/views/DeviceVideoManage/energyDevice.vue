<template>
	<div class="energyDevice">
		...
		<div class="energyDevice_top">
			<!--$router.replace('/deviceVideoManage')-->
				...
				<li>
					...
					<div class="common_select">
						<el-select v-model="careerValue" @change="careerChange" placeholder="请选择">
							<el-option v-for="item in careerOptions" :key="item.value" :label="item.label" :value="item.value">
							</el-option>
						</el-select>
					</div>
				</li>
		...
		<div class="energyDevice_main">
			<div class="energyDevice_left" id="energyDeviceLeft">
				<div class="energyDevice_leftHead">
					...
					<!--左侧弹窗开始-->
					<div class="centername" @click="isClosedCenternameDialog=false" v-show="centerDialogShow">
					...
					<div class="common_select" v-show="jgldShow">
						<el-form label-positio="top" label-width="80px;" :inline="true">
					...
					<div style="width:100%;height:670px;" id="energyDeviceTable">
						<el-scrollbar style="height:100%;width:100%;">
							<table style="width:100%;100%;" v-if="deviceList.length>0">
								<tbody>
									<!-- class="selectedTr" -->
									<tr v-for="(item) in deviceList" :key="item.num" :class="{selectedTr:currentDeviceObj.num==item.num}" @click="handleSeletedCurrentDevice(item)">
			...
			<div class="energyDevice_con">
				...
				<div class="energyDevice_conBottom">
					<div class="energyDevice_paigong">
						...
						<div class="content" style="text-align: center;padding-top: 70px;font-size: 14px;" v-if="currentDeviceObj.paigongList&&currentDeviceObj.paigongList.length<=0">暂无派工</div>
						<div class="content" v-else>
							...
							<div class="energyDevice_paigong_tbody">
								<el-scrollbar style="height:100%;width:100%;">
									<div class="energyDevice_paigong_td" v-for="(item, index) in currentDeviceObj.paigongList" @mouseover="handleCurrentPaigongInfo(item)"
									 @mouseout="isFocusPaigong=false" :class="{selectedTr:(isFocusPaigong&&currentPersonWorkInfo.workNo==item.workNo)||isFocusPaigongPopup&&currentPersonWorkInfo.workNo==item.workNo}"
									 :key="index">
								...
								<div class="tipComponentWrapper" @mouseover="isFocusPaigongPopup=true" @mouseout="isFocusPaigongPopup=false"
					...
					<div class="energyDevice_dianLiu">
						<div class="top">
							...
							<div class="dianbiao">
								<el-select class="common_select" v-model="dianbiaoValue" placeholder="请选择" @change="dianbiaoChange" size="mini">
									<el-option v-for="item in dianbiaoOptions" :key="item.value" :label="item.label" :value="item.value">
									</el-option>
								</el-select>
							</div>
							<ul class="dianliuTab">
								<li v-for="(item) in tabDian" :key="item.id" :class="{spantab:true,current:activespan==item.id}" @click="spantab(item.id)">{{item.message}}</li>
							</ul>
						</div>
						<div class="bottom">
			...
			<div class="energyDevice_right" v-show="centerDialogShow">
		...
		<!-- 二级机构 -->
		<!-- :class="{off:isClosedCenternameDialog}" -->
		<!-- <div class="home_container" :class="{off:isClosedSecondDialog}">
	  <header class="home_container_title" @click="getCenterInfo1('')">
	    北京桩机
	  </header>
	  <img class="close" src="../../assets/images/close.png" @click="isClosedSecondDialog=true" />
	  <ul class="home_container_list">
	    <li v-for="(centerItem,index) in allCenterList" :key="index" :ref="index" @click="getCenterInfo1(centerItem)">{{centerItem}}</li>
	  </ul>
	</div> -->
		<!-- 二级机构end -->
...
<script>
	...
	export default {
		...
		computed: {
			centerDialogShow() {
				return this.$store.state.centerDialogShow
			},
			jgldShow() {
				return this.$store.state.jgldShow
			}
		},
		mounted() {
			// 顶部日期时间
		...
		methods: {
			...
			async getOptions(num, type, message, url) {
				...
				if (res && res.data.ret == 200) {
					this.secondeGroupOptions = res.data.data.map((item, index) => {
						return {
			...
			handleBack() {
				this.$router.replace('/deviceVideoManage');
			...
			getAllRequest() {
				...
				// 左侧表格数据
				if(sessionStorage.groupCodeSelf){
					this.getDataList1()
				}else{
					this.getDataList()
				}
			...
			/*
			 getCenterInfo1 (item) {
			  // 调用发请求
			  this.isClosedSecondDialog = true
			  //this.centerName = item
			  this.pagination.currentPage = 1
			  this.getAllRequest()     
			},
			*/
			...
			getRecordRadarChart2(dataObj) {
				// var inDicatorArr = 
				let yichangNum = parseFloat(dataObj.lateNum) + parseFloat(dataObj.absentNum) + parseFloat(dataObj.outNum) +
					parseFloat(dataObj.abnormalNum)
				let zhengchangNum = parseFloat(dataObj.totalNum) - parseFloat(dataObj.lateNum) - parseFloat(dataObj.absentNum) -
					parseFloat(dataObj.outNum) - parseFloat(dataObj.abnormalNum)
				...
				let totalLv = parseFloat(yichangLv) + parseFloat(paigongLv) + parseFloat(shanggangLv) + parseFloat(zaigangLv) +
					parseFloat(chuqinLv)
			...
			*/
			async getDataList1() {
				const res = await getDeviceByGY({
					centerName: '',
					secondGroupCode: this.secondGroupValue==''?sessionStorage.groupCodeSelf:this.secondGroupValue,
					page: this.pagination.currentPage,
					pageSize: this.pagination.pageSize
				});
				if (res && res.status == 200 && res.data.ret == '200') {
