  ...
  <div class="index">
    ...
    <div class="index_top">
      ...
      <div class="leftInfo">
        ...
        <div class="back"><img src="../../assets/images/index_back.png"></div>
...
import Attendance from '@/components/timeChecking/Attendance'    // 日月年echarts
import EnergyStatistics from '@/components/timeChecking/EnergyStatistics'  // 能源指标列表echarts
import Checking from '@/components/timeChecking/Checking.vue'
import MachingCenter from '@/components/timeChecking/MachingCenter'
import HeaderToSelected from '@/components/timeChecking/TimeHeaderToSelected'    //头部下拉选项
import CheckingLeftTopNum from '../../components/timeChecking/CheckingLeftTopNum' //20190517 左上考勤人数
import CheckingLeftCenterEcharts from '../../components/timeChecking/CheckingLeftCenterEcharts' //20190517左中考勤echarts图表
...
export default {
  ...
  mounted () {
    ...
    // let subompanyName = localStorage.getItem('companyNameCheckingSelectedSubcompany')20190521暂时写死^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    let subompanyName = '北京总部'
    // axios.defaults.baseURL = localStorage.getItem('ipAddrCheckingSelectedSubcompany')20190521
    let baseUrl = 'http://10.19.8.22:8099'
    axios.defaults.baseURL = baseUrl
    localStorage.setItem('ipAddrCheckingSelectedSubcompany',baseUrl)
    // if(subompanyName){
    //   subompanyName = localStorage.setItem('companyNameCheckingSelectedSubcompany')
    // }
    this.$store.commit('changeSubcompanyMutTime',subompanyName)
  ...
  methods: {
    ...
    async getAbsentLateLeaveChangeworkList () {
      ...
      const resultAbsent = await getAbsentList0417(1,currentTimeDate,3,'','',centerNameFromCentername,1,1000)
      ...
      const resultDispatchList = await getLateList0417(1,currentTimeDate, 1, '','',centerNameFromCentername,1, 1000)
      ...
      const resultShiftList = await getChangeWorkList0417(1,currentTimeDate, 2,'','',centerNameFromCentername, 1, 1000)
    ...
    // 回到历史页面
    enterCheckHistory () {
      this.$router.replace('/TimeCheckingHistoryData')
    },
...
.index{
  ...
  &_top{
    ...
    height: 106px;line-height:45px;text-align: center;
    .title {
      font-size: 0.32rem;
    ...
    .historyBtn{
      ...
      margin-top: 5px;margin-left:10px;cursor: pointer;
      background:linear-gradient(#176275,#06437d);border-radius: 3px;
    }
    .leftInfo{
      position: absolute;top:20px;left:0;overflow: hidden;
    }
  }
  &_main{
