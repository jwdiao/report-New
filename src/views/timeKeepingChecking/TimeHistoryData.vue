  ...
  <div class="index">
    ...
    <div class="index_top">
      <div class="index_imgText">
        <span class="title" style="cursor:pointer;" @click="switchHistoryData()">{{this.$store.state.selectedSubcompanyTime}}考勤数据</span>
      </div>
      <em class="time" v-text="currentTime"></em>
      <div class="button" @click="enterIndexPage('/TimeChecking')"></div>
      <!--日期start-->
...
<script>
  ...
  import HeaderToSelected from '../../components/timeChecking/TimeHeaderToSelected.vue'
  import {
    reqnSanyHistoryData
  }from'../../api'
  ...
  export default {
    ...
    methods: {
      ...
      //点击跳转页面
      skipToHistory(date){
        this.$router.replace({name:'TimeCheckingHistoryPage',params:{dateData:date}})
      },
...
<style lang="scss" scoped>
  .index{
    ...
    /*6s页面上面盒子*/
    &_center{
      width: 100%;float: left;min-height:calc(100% - 15px);margin-right: 15px;
      /*margin-top: 15px;*/
