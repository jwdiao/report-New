		...
		methods: {
			...
			async getEventCount() {
				...
				if (res.data && res.data.ret == 200) {
					...
					this.renderClock(document.getElementById('chart1'), {
						name: '物料乱放',
						value: this.wllfuml
					}); //物料乱放echarts函数调用
					this.renderClock(document.getElementById('chart2'), {
						name: '安全帽',
						value: this.aqmnuml
					}); //安全帽函数调用
					this.renderClock(document.getElementById('chart3'), {
						name: '核心区域进出',
						value: this.hxqyjcnuml
					}); //核心区域进出函数调用
					this.renderClock(document.getElementById('chart4'), {
						name: '物料取走',
						value: this.wlqznuml
					}); //物料取走函数调用
