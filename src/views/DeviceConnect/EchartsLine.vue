  ...
  methods: {
    async renderDianliu() {
      ...
      var option = {
        ...
        // if(!dataArr){ return; }
        let dataArr = []
        const response = await http.post('http://10.19.8.22:9099/machineConnection/selectByDeviceID', { machineNo: this.deviceID })
        if(response && response.data.code === 200){
            dataArr = (response.data.data.deviceResponseDtos).reverse()
        } else {
            return false
        }
        let xAxisData = []
    ...
    //   return;
      this.setIntervalId = setInterval(async () =>{
        ...
        if(responseOneSecond && responseOneSecond.data && responseOneSecond.data.code === 200 && responseOneSecond.data.data&& responseOneSecond.data.data.deviceResponseDtos && responseOneSecond.data.data.deviceResponseDtos.length>0){

            currentSecondData = responseOneSecond.data.data.deviceResponseDtos[0]
        } else {
            return false
        }
        // debugger;
