<template>
  <div class="maching-center">
    <div class="home_head-btn" @click="bostmachcenter">
      <span v-show="$store.state.centername===''">{{this.$store.state.selectedSubcompanyTime}}</span>
    ...
    <div class="home_container" :class="{off:flag}">
      <header class="home_container_title" @click="getCenterInfo('')">
        {{this.$store.state.selectedSubcompanyTime}}
      </header>
...
export default {
  ...
  data () {
    return {
      titleText: '',
      flag: true, // 加工中心显示
      currentCenterName: this.$store.state.selectedSubcompanyTime
    }
  },
  mounted () {
  ...
  methods: {
    ...
    getCenterInfo (item) {
      ...
      if (item === '') {
        this.currentCenterName = this.$store.state.selectedSubcompanyTime
      }
...
.maching-center{
  ...
  .home_head-btn{
    // width:1.4rem;
    width:190px;
    position: fixed;right:24px;top:22px;height:40px;text-align: center;line-height: 40px;
