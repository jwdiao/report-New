            ...
            <div class="checking_energyTable" v-show="$store.state.centername!==''">
              <table>
                ...
                <tr v-for="(item, index) in $store.state.energyListData.list" :key="index">
                  <td>{{index+1}}</td>
                  <!-- <td>{{($store.state.energyListData.page-1)*10+index+1}}</td> -->
...
export default {
  name: 'checking',
  props: ['info', 'isDayOrNigint', 'kaoqinList', 'subCompany'],
  watch: {
    info () {
...
<style lang="scss" scoped>
  .checking{
    ...
    // 20190108
    &_wrapper{
      flex:2;margin-top:15px;overflow: hidden;font-size:12px;
      ...
      .checking_energy{
        ...
        .checking_item-title{
          color:#53e3fd;font-size:16px;font-weight: bold;
          height:50px;line-height: 50px;
          text-align: left;padding:0 5px;
          background-color:#2a4b85;overflow: hidden;
          .numAndUnit{float: right;}
        }
        .checking_item-wrapper{
          background-color: rgba(42, 75, 133, 0.3);
          font-size:16px;
        ...
        .checking_energyTable{
          height:100%;
          table{width:99.8%;text-align: center;}
          th{
            background-color: #2a4b85;
            color:#4095fe;text-align: center;
            font-size: 16px;
            font-weight: bold;
