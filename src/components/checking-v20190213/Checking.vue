	...
	<div class="checking">
		...
		<div class="checking_wrapper">
			<el-tabs class="checking_tabs" v-model="currentTab" @tab-click="handleCheckingTab">
				...
				<el-tab-pane label="人员考勤雷达图" name="leida" v-if="$store.state.centername===''">
...
<script>
	...
	export default {
       ...
       count(){
			 ...
			 centerName(){
				 return this.$store.state.centername
			 }
		},
		created() {},
		mounted() {
      ...
      getRecordRadarChart(myArr){
				...
				if(myArr instanceof Array){
					/* myArr.forEach((ele,index)=>{
							ele.totalYiChang = ele.lateNum+ele.outNum+ele.absentNum+ele.abnormalNum
					})
					myArr.sort(function(a,b){
								return b.totalYiChang - a.totalYiChang
					}) */
					var myRadarCharts = [];
					this.fuZhiBiaoList = [];
					this.indicatorList = [];
					...
					/* this.zhengZhiBiaoList.push([ele.totalNum - ele.lateNum, ele.totalNum - ele.outNum, ele.totalNum - ele.absentNum,
					    ...
					    myRadarCharts.push(ele.leaveNum,ele.outNum,ele.absentNum, ele.abnormalNum)
					 ...
					 console.log(Math.max.apply(null,myRadarCharts))
					this.indicatorList.forEach((ele,index)=>{
						  ele.forEach((ele,index)=>{
								ele.max = Math.max.apply(null,myRadarCharts)
								//ele.max = 10
							})
					})
					var that = this
