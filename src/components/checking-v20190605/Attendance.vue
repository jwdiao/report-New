<template>
  <div class="attendance">
    <div class="attendance-item" style="padding-top: 15px">
      <!--<p class="home_title home_title-blue">人员考勤日统计</p >-->
      <p class="home_title titleText">人员考勤日统计</p >
      <div class="day" id="checkingv20190227-attendance_day"></div>
    </div>
    <div class="attendance-item">
      <!--<p class="home_title home_title-yellow">人员考勤月统计</p >-->
      <p class="home_title titleText">人员考勤月统计</p >
      <div class="month" id="checkingv20190227-attendance_month"></div>
    </div>
    <div class="attendance-item" style="padding-bottom: 15px">
      <!--<p class="home_title home_title-purple">人员考勤年统计</p >-->
      <p class="home_title titleText">人员考勤年统计</p >
      <div class="year" id="checkingv20190227-attendance_year"></div>
    </div>
  </div>
</template>
...
export default {
  name: 'Attendance',
  props: ['dayInfo', 'monthInfo', 'yearInfo','yearData'],
  watch: {
    //异常年统计
    yearData(val){
      this.yearABEchartsRightData = {
        cidao : val.lateArr, // 迟到
        zaotui : val.leaveNumArr, // 早退
        kuanggong : val.absenNumArr, //矿工
        abnormalnum:val.abnormalnum,//未派工
      }
      this.renderYearEcharts(this.yearEchartsRight,this.yearEchartsRightData, this.yearABEchartsRightData) // 渲染echarts图
      console.log('this.yearEchartsRightData:',this.yearEchartsRightData)
    },
    // 人员考勤日统计数据
    dayInfo (val) {
      console.log('val::::',val)
      ...
      if (this.dayInfo && this.dayInfo.ret == 200) {
        ...
        this.dayEchartsRightData = {
          ...
          shanggangren: val.practical, // 上岗人数
					queqinrenshu:val.missRecordNum,//异常人数
          abnormalnum:val.abnormalnum,//未派工
          absentnum:val.absentnum,//未到
          latenum:val.latenum,//迟到
          outnum:val.outnum,//离岗
  ...
  data () {
    return {
      ...
      yearABEchartsRightData:{},
      EchartsArr: [] //所有echarts图表数组
    }
  },
  mounted () {
    // 日统计
    this.dayEchartsRight = echarts.init(document.getElementById("checkingv20190227-attendance_day"))
    this.EchartsArr.push(this.dayEchartsRight) // 放入echarts数组

    // 月统计
    this.monthEchartsRight = echarts.init(document.getElementById("checkingv20190227-attendance_month"))
    this.EchartsArr.push(this.monthEchartsRight) // 放入echarts数组

    // 年统计
    this.yearEchartsRight = echarts.init(document.getElementById("checkingv20190227-attendance_year"))
  ...
  methods: {
    ...
    renderYearEcharts(myChart, data,Data){
      var option = {
        tooltip: {
          trigger: 'axis',
          axisPointer: {
...
<style lang="scss" scoped>
  ...
  &-item{
    flex:1;
    padding:0 15px;
    .home_title{width:100%;font-size: 0.22rem}
    .day{
