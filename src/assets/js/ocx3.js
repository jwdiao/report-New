...
export function InitSpvx(ocxObj) {
	var languageType = 1, ret = ocxObj.MPV_Init(languageType);
	return ret;
}
...
export function SetToolBar(ocxObj) {
	var tbars = '0,1,2,3,4,5,6', ret = ocxObj.MPV_SetToolBar(tbars);
	if (ret != 0) {
		//alert("设置工具条失败！");
	}
...
export function playVideoByOcx(spvxOcx, url) {
	var ret = spvxOcx.MPV_StartPreview(url);
	return ret;
}
...
export function stopPreview(ocxObj) {
	var ret = ocxObj.MPV_StopPreview();
	return ret;
}
 ...
 * @returns
 */
export function MPV_SetOSDText(ocxObj, xml) {
